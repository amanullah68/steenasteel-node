...
txConfirmation: (req, res) => {
	var active = 1;
	var stop_exec = 2;
	var sql = "SELECT * FROM supplier_certificates WHERE blockchain_confirmation = "+stop_exec+"";
	connection.query(sql, function (err, results) { 
		if (err) {
			...
			if(results.length>0) {
				...
				var sql = "SELECT *FROM supplier_certificates WHERE blockchain_confirmation = "+0+" AND isActive = "+active+" AND blockchain_executed = "+active+" AND (countt=0 OR countt=1) ORDER BY id ASC LIMIT 1";
				connection.query(sql, function (err, results) {
					if (err) {
						console.log(err);
					}
	...
	postToApi.postToApi('ethereum/contract/getTransactionReceipt',params,async function(err, body){
		...
		if (err) {
			...
			if(body.errorcode) {
				...
				if(st == 0) {
					var sql = "UPDATE supplier_certificates SET blockchain_executed = "+2+" WHERE id = "+id+"";
				}
				else {
					var sql = "UPDATE supplier_certificates SET blockchain_executed = "+2+" WHERE id = "+id+"";
				}
...
const updateTXcofirmationStatus = async (setStatus, id, flag) => {
	try {
		if(id){
			var sql = "UPDATE supplier_certificates SET blockchain_confirmation = "+setStatus+" WHERE id = "+id+"";
		}
		await connection.query(sql, (err, results) => {
            if (err){
				flag(err);
